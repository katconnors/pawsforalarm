...
def create_animal(
    ...
    join_date=None,
    weight=None,
    scheduled_euthanasia_date=None,
    bio=None,
):
...
def view_animals(
    type="all", query_state="all", group="all", sort_type="scheduled_euthanasia_date"
):
    ...
    if query_state and query_state != "all":
        ani_list = ani_obj.filter(
            Shelter.state == query_state, Animal.status == "available"
        )
...
def animal_type_list():
    ...
    animaltype_list = (
        database.session.query(Animal.type).distinct().order_by(Animal.type).all()
    )
...
def shelter_state_list():
    ...
    state_list = (
        database.session.query(Shelter.state).distinct().order_by(Shelter.state).all()
    )
